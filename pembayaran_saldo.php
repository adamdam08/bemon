...
<form action="insert_pembayaran_engine_vsaldo.php" enctype="multipart/form-data" method="post">
	<div class="col-md-12">
		<div class="rsearch">
			...
			<button style="float:right;margin-top:-2.3%;margin-bottom:2.5%;margin-left:5px;" onclick="window.location.href = 'http://localhost/ukk_real/pembayaran.php?book=<?php echo $keyword1?>'" type="button" class="btn btn-primary ">Transfer</button>
		</div>
	</div>
	<?php
		...
		<div class="rsearch">
			...
			<p>Discount :</p>
				...
				<span class="input-group-addon">
					...
					<input type="text" readonly value="10%" name="id_book" class="form-control">
				</div>	
			<br/>
			...
			<br/>
			<?php $waw = $data[total] - ( $data[total] * (10 / 100)) ?>
			...
			<div class="rsearch">
				...
				<hr/>
						...
						<br/>
							<?php if($dasa[saldo] >= $data[total]) {?>
								<button style="float:right;" type="submit" class="btn btn-primary"><i>Bayar</i></button> 
							<?php }else{ ?>
							<button style="float:right;" type="button" class="btn disabled btn-primary"><i>Bayar</i></button> 
							<?php } ?>
						<br/>
						<br/>
					</form>
